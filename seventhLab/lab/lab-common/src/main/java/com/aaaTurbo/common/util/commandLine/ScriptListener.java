...
public class ScriptListener {
    ...
    public ScriptListener(DatagramSocket cl, String user, String password) throws UnknownHostException {
    ...
    private void executeOtherScriptWirhoutRecurs(String[] args) throws Exception {
        ...
        executeScript.setPassword(password);
        if (executeScript.validate(args)) {
            executeScript.execute(args, client.getLocalPort(), user);
        }
    }
